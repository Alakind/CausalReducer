...
class CausalReducer:
    ...
    state_transition_traces: Dict[str, List[Transition]] = {}
    ...
    def reduce_single_bad_state(self, bad_state: State):
    ...
    def reverse_bfs_step(self, current_state: State, transitions: List[Transition]):
        if current_state.id in self.state_transition_traces:
            if not self.is_trace_subset(transitions, self.state_transition_traces[current_state.id]):
        ...
        for transition in current_state.transitions_target:
            self.bfs_queue.append((self.statespace[transition.state_from_id], transitions + [transition]))
            # self.reverse_bfs_step(self.statespace[transition.state_from_id], transitions + [transition])
    ...
    def is_trace_subset(self, transitions_A: List[Transition], transitions_B: List[Transition]):
